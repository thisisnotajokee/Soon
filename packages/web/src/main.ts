...
type ViewId = 'tracking' | 'deals' | 'add' | 'notifications' | 'settings';
type DetailTab = 'overview' | 'settings';
type Lang = 'pl' | 'en' | 'de';
type TrackingStatusFilter = 'active' | 'all' | 'inactive';
...
function marketFlag(domainRaw: string | null | undefined) {
...
function currencyForDomain(domainRaw: string | null | undefined) {
  return String(domainRaw || '').toLowerCase() === 'uk' ? '£' : '€';
}

function formatPrice(value: unknown, domainRaw?: string | null) {
...
function sortedMarkets(rows: MarketRow[] = []) {
...
function normalizedRows(item: TrackingItem) {
...
function bestNewPrice(item: TrackingItem) {
...
function avgNewPrice(item: TrackingItem) {
...
function isSnoozed(item: TrackingItem) {
...
function buildDetailChartRows(item: TrackingItem, detail: ProductDetail | null) {
  const rowsByKey = new Map<string, DetailChartRow>();
  ...
  const pricesByMarket = new Map(
    normalizedRows(item)
      .map((row) => [String(row.market || '').toLowerCase(), Number(row.newPrice)] as const)
      .filter(([, price]) => Number.isFinite(price) && price > 0),
  );
  const existingMarkets = new Set(existingRows.map((row) => String(row.domain || '').toLowerCase()));
  const referenceMarket =
    (DETAIL_CHART_ORDER.find((market) => existingMarkets.has(market)) || [...existingMarkets][0] || '') as string;
  ...
  const safeReference =
    Number.isFinite(referenceLast) && Number(referenceLast) > 0
...
function applyDetailChartFilters(rows: DetailChartRow[], selectedMarkets: string[], rangeDays: number) {
...
function buildDetailChartDatasets(rows: DetailChartRow[]) {
  const datasets: any[] = [];
...
function computeDetailChartStats(rows: DetailChartRow[]) {
  const points = rows
    .map((row) => {
...
function renderDetailChartCanvas(filteredRows: DetailChartRow[]) {
...
function normalizeDetailHistorySeries(item: TrackingItem, detail: ProductDetail | null) {
  const fromDetail = Array.isArray(detail?.historySeries) ? detail?.historySeries || [] : [];
  const byMarket = new Map<string, Array<{ ts: string; tsMs: number; value: number }>>();
  ...
  const referenceMarket =
    (pricedMarkets.find((row) => byMarket.has(row.market))?.market || byMarket.keys().next().value || '') as string;
...
function getDetailChartMarkets(asinRaw: string, availableMarkets: string[], bestDomainRaw: string) {
...
function detailChartSvg(
  seriesByMarket: Map<string, Array<{ ts: string; tsMs: number; value: number }>>,
  selectedMarkets: string[],
  rangeDays: number,
  bestDomainRaw: string,
) {
  ...
  if (!selected.length) {
    return {
      ...
      stats: { min: null as number | null, max: null as number | null, avg: null as number | null, vol: null as number | null, sinceMinDays: null as number | null },
  ...
  const toX = (tsMs: number) => left + ((tsMs - minTs) / spanTs) * plotW;
  const toY = (value: number) => bottom - ((value - minValue) / spanValue) * plotH;

  const seriesPaths = filtered
    .map(([market, points]) => {
      const xy = points.map((point) => ({ x: toX(point.tsMs), y: toY(point.value), ts: point.ts, value: point.value }));
      if (xy.length < 2) return '';
      const pathParts: string[] = [`M${xy[0].x.toFixed(2)} ${xy[0].y.toFixed(2)}`];
  ...
  const statVol =
    Number.isFinite(Number(statMin)) && Number.isFinite(Number(statMax)) && Number.isFinite(Number(statAvg)) && Number(statAvg) > 0
...
function chartToken(points: SparkPoint[], prefix: string) {
...
function sparklineSvg(points: SparkPoint[], stroke = '#ff7a00', domainRaw = 'de') {
  if (!points.length) {
...
function formatChartLabelDate(value: string) {
  const ts = Date.parse(String(value || ''));
  if (!Number.isFinite(ts)) return '—';
  return new Intl.DateTimeFormat(state.lang, {
    day: 'numeric',
...
function formatChartAxisPrice(value: number, domainRaw: string) {
...
function cardHistorySvg(points: SparkPoint[], domainRaw: string) {
  if (!points.length) {
...
function openAmazon(asinRaw: string, marketRaw: string) {
...
function updateTrackingStatusFiltersUi() {
  ...
  for (const button of root.querySelectorAll<HTMLButtonElement>('.deal-chip[data-status]')) {
...
function recommendationBadge(item: TrackingItem) {
  const copy = t();
...
function renderTrackingCard(item: TrackingItem) {
  const copy = t();
  ...
  const rowByMarket = new Map(rows.map((row) => [String(row.market || '').toLowerCase(), row] as const));
...
function renderPriceRows(rows: Array<[string, number]>, type: 'new' | 'used', asin: string) {
...
function renderMarketCompare(rows: Array<[string, number]>, bestValue: number | null, asin: string) {
  if (!rows.length) return `<div class="detail-overview-value">${escapeHtml(t().noData)}</div>`;
...
async function hydrateDetail(asinRaw: string) {
  const asin = String(asinRaw || '').trim().toUpperCase();
  if (!asin) return null;
  if (state.detailByAsin[asin]) return state.detailByAsin[asin];
  try {
    const detail = (await client.getProductDetail(asin)) as ProductDetail;
...
function renderDetail(item: TrackingItem) {
  ...
  const rows = normalizedRows(item);
  const newRows = rows
    .map((row) => [String(row.market || '').toLowerCase(), Number(row.newPrice)] as [string, number])
    .filter(([, value]) => Number.isFinite(value) && value > 0)
    .sort((a, b) => a[1] - b[1]);

  const usedRows = rows
    .map((row) => [String(row.market || '').toLowerCase(), Number(row.usedPrice)] as [string, number])
    .filter(([, value]) => Number.isFinite(value) && value > 0)
    .sort((a, b) => a[1] - b[1]);

  const image = String(item.cardPreview?.imageUrl || item.imageUrl || '').trim();
  const detailChartRows = buildDetailChartRows(item, detail);
  const availableChartMarkets = DETAIL_MARKETS.filter((market) =>
    detailChartRows.some((row) => String(row.domain || '').toLowerCase() === market),
  );
  ...
  nodes.detailContent.innerHTML = `<div class="dhdr">
    <div class="dhdr-img"><span class="dhdr-img-fallback material-icons-round" aria-hidden="true">inventory_2</span>${
      image
  ...
  <div class="dtpanel ${state.detailTab === 'overview' ? 'on' : ''}" id="dt-overview">
    <div class="detail-section-title-gap"><span class="material-icons-round">compare_arrows</span><span>Porównanie między rynkami</span></div>
...
async function openDetail(asinRaw: string) {
  const asin = String(asinRaw || '').trim().toUpperCase();
...
async function refreshSelectedTracking(asinRaw: string) {
  const asin = String(asinRaw || '').trim().toUpperCase();
  if (!asin) return;
  try {
...
async function snoozeSelectedTracking(asinRaw: string) {
  const asin = String(asinRaw || '').trim().toUpperCase();
  if (!asin) return;
  try {
...
async function unsnoozeSelectedTracking(asinRaw: string) {
  const asin = String(asinRaw || '').trim().toUpperCase();
  if (!asin) return;
  try {
