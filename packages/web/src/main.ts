import type { ViewId, DetailTab, Lang, TrackingStatusFilter, DealsSource, DealsSort, TrackingSort, NotifTab, AddMode, MarketRow, SparkPoint, DetailHistoryPoint, ProductDetail, CardPreview, SnoozeState, TrackingItem, DetailChartRow } from './state/types.js';
import { client } from './services/instance.js';
import { state, MARKET_ORDER, DETAIL_MARKETS, TRACKING_CARD_MARKETS, DETAIL_CHART_ORDER } from './state/index.js';
import { nodes } from './utils/dom.js';
import { t, setQueryParam } from './utils/i18n.js';
import {
  escapeHtml, marketFlag, formatPrice, formatDateTime, currencyForDomain,
  toNullableNumber, parseAsinFromInput,
  sortedMarkets, normalizedRows, bestNewPrice, bestUsedPrice,
  getBestDomain, avgNewPrice, isSnoozed, getSparkline, marketStrokeColor,
...
import {
  openDetail, closeDetail, refreshSelectedTracking,
  snoozeSelectedTracking, unsnoozeSelectedTracking,
  deleteSelectedTracking, shareSelectedTracking, copyMobileUrl,
...
function setActiveView(next: ViewId) {
  closeDetail();
  if (state.activeView !== next) {
    telemetryLog('navigate.view', { from: state.activeView, to: next });
  }
  state.activeView = next;
  for (const [key, view] of Object.entries(nodes.views) as Array<[ViewId, HTMLElement | null]>) {
    if (!view) continue;
...
async function submitAddProduct() {
  if (!nodes.asinInput) return;
  const raw = nodes.asinInput.value;
  const asins = parseAsinFromInput(raw);
  if (!asins.length) {
    if (nodes.addResultBox) nodes.addResultBox.textContent = 'Wklej poprawny ASIN lub link Amazon.';
    return;
  }
  telemetryLog('product.add', { count: asins.length, mode: state.addMode, domains: state.addDomains });
  if (nodes.bulkProgress) nodes.bulkProgress.classList.remove('settings-hidden');
  const results: string[] = [];
  for (let i = 0; i < asins.length; i += 1) {
    const asin = asins[i];
    if (nodes.bulkBarFill) nodes.bulkBarFill.style.width = `${((i + 1) / asins.length) * 100}%`;
    if (nodes.bulkStatus) nodes.bulkStatus.textContent = `Dodawanie ${asin}…`;
    try {
  ...
  if (nodes.bulkBarFill) nodes.bulkBarFill.style.width = '100%';
  if (nodes.bulkStatus) nodes.bulkStatus.textContent = 'Gotowe';
  ...
  setTimeout(() => {
    if (nodes.bulkProgress) nodes.bulkProgress.classList.add('settings-hidden');
  }, 1500);
  await loadTrackings();
}
...
function bindEvents() {
  for (const button of nodes.navItems) {
    button.addEventListener('click', () => {
      const view = String(button.dataset.v || 'tracking') as ViewId;
      setActiveView(view);
    });
  }
  ...
  nodes.trackingStatusFilters?.addEventListener('click', (event) => {
    ...
    const button = target.closest<HTMLButtonElement>('.deal-chip[data-status]');
    if (!button) return;
    const status = String(button.dataset.status || '') as TrackingStatusFilter;
  ...
  nodes.productsList?.addEventListener('click', (event) => {
    ...
    const marketButton = target.closest('[data-action="open-market"]') as HTMLButtonElement | null;
    if (marketButton) {
      openAmazon(marketButton.dataset.asin || '', marketButton.dataset.market || 'de');
      return;
    }

    const bestButton = target.closest('.pcard-price-row[data-asin][data-market]') as HTMLElement | null;
    if (bestButton) {
      openAmazon(bestButton.dataset.asin || '', bestButton.dataset.market || 'de');
      return;
    }

    const gridItem = target.closest('.pgrid-item[data-domain][data-asin]') as HTMLElement | null;
    if (gridItem) {
      openAmazon(gridItem.dataset.asin || '', gridItem.dataset.domain || 'de');
      return;
    }

    const card = target.closest('.pcard[data-asin]') as HTMLElement | null;
  ...
  nodes.detailContent?.addEventListener('click', async (event) => {
    ...
    const bestButton = target.closest('[data-action="open-best"]') as HTMLButtonElement | null;
    if (bestButton) {
      openAmazon(bestButton.dataset.asin || '', bestButton.dataset.market || 'de');
      return;
    }

    const tabButton = target.closest('.dtab[data-dt]') as HTMLElement | null;
    if (tabButton) {
      const tab = String(tabButton.dataset.dt || 'overview') as DetailTab;
    ...
    const rangeButton = target.closest('#timeRange [data-r]') as HTMLButtonElement | null;
    if (rangeButton) {
    ...
    const chartMarketChip = target.closest('[data-action="toggle-chart-market"]') as HTMLElement | null;
    if (chartMarketChip) {
    ...
    const refreshButton = target.closest('[data-action="refresh-asin"]') as HTMLButtonElement | null;
    if (refreshButton) {
      await refreshSelectedTracking(refreshButton.dataset.asin || '');
      return;
    }

    const snoozeButton = target.closest('[data-action="snooze"]') as HTMLButtonElement | null;
    if (snoozeButton) {
      await snoozeSelectedTracking(snoozeButton.dataset.asin || '');
      return;
    }

    const unsnoozeButton = target.closest('[data-action="unsnooze"]') as HTMLButtonElement | null;
    if (unsnoozeButton) {
      await unsnoozeSelectedTracking(unsnoozeButton.dataset.asin || '');
      return;
    }

    const saveThresholdsButton = target.closest('[data-action="save-thresholds"]') as HTMLButtonElement | null;
    if (saveThresholdsButton) {
      const asin = String(saveThresholdsButton.dataset.asin || '').trim().toUpperCase();
      if (!asin || !nodes.detailContent) return;
      const dropPctInput = nodes.detailContent.querySelector<HTMLInputElement>('#d-drop-pct');
      const risePctInput = nodes.detailContent.querySelector<HTMLInputElement>('#d-rise-pct');
      const targetNewInput = nodes.detailContent.querySelector<HTMLInputElement>('#d-target-new');
      const targetUsedInput = nodes.detailContent.querySelector<HTMLInputElement>('#d-target-used');
    ...
    const detailMarketToggle = target.closest('[data-action="toggle-detail-market"]') as HTMLElement | null;
  ...
  nodes.detailContent?.addEventListener('keydown', (event) => {
    ...
    const target = event.target as HTMLElement;
    const actionable = target.closest<HTMLElement>('[role="button"][data-action], #timeRange [data-r]');
  ...
  nodes.langRow?.addEventListener('click', async (event) => {
    const target = event.target as HTMLElement;
    const button = target.closest('.mchip[data-lang]') as HTMLButtonElement | null;
    if (!button) return;

    const next = String(button.dataset.lang || 'pl') as Lang;
  ...
  for (const button of nodes.navItems) {
    button.addEventListener('click', async () => {
      const view = String(button.dataset.v || 'tracking') as ViewId;
      if (view === 'deals') await loadDeals();
      if (view === 'notifications') await loadAlerts();
      if (view === 'settings') await refreshAllSettingsData();
    });
  }
  ...
  nodes.dealsQuickFilters?.addEventListener('click', (event) => {
    const target = event.target as HTMLElement;
    const chip = target.closest<HTMLButtonElement>('.deal-chip[data-deals-source]');
    if (!chip) return;
    const source = String(chip.dataset.source || '') as DealsSource;
  ...
  nodes.dealsFabMenu?.addEventListener('click', async (event) => {
    ...
    if (action === 'set-deals-sort') {
      const sort = String(actionBtn.dataset.sort || '') as DealsSort;
  ...
  nodes.trackingFabMenu?.addEventListener('click', (event) => {
    ...
    if (action === 'set-tracking-sort') {
      const sort = String(actionBtn.dataset.sort || '') as TrackingSort;
      if (!sort) return;
      state.trackingSort = sort;
      nodes.trackingFabMenu?.classList.remove('open');
      for (const btn of nodes.trackingFabMenu?.querySelectorAll<HTMLButtonElement>('.deals-fab-action[data-sort]') || []) {
  ...
  nodes.addModeSwitch?.addEventListener('click', (event) => {
    const target = event.target as HTMLElement;
    const chip = target.closest<HTMLButtonElement>('.add-mode-chip[data-add-mode]');
    if (!chip) return;
    state.addMode = String(chip.dataset.addMode || 'quick') as AddMode;
    updateAddModeUi();
  });

  nodes.addDomains?.addEventListener('click', (event) => {
    const target = event.target as HTMLElement;
    const chip = target.closest<HTMLDivElement>('.domtoggle[data-d]');
    if (!chip) return;
  ...
  document.querySelector('#v-add')?.addEventListener('click', async (event) => {
    ...
    if (!actionBtn) {
      ...
      const chip = target.closest<HTMLButtonElement>('button[data-add-preset], button[data-price-type], button[data-track-mode], button[data-drop-pct], button[data-rise-pct], button[data-stock-event]');
      if (chip) {
        ...
        if (preset) {
          ...
          const map: Record<string, { drop: number | null; rise: number | null }> = { safe: { drop: 20, rise: 15 }, standard: { drop: 10, rise: 5 }, aggressive: { drop: 5, rise: 1 } };
          state.addDropPct = map[preset]?.drop ?? 10;
          state.addRisePct = map[preset]?.rise ?? 5;
        }
        if (priceType) state.addPriceType = priceType;
        if (trackMode) state.addTrackMode = trackMode;
        if (dropPct !== undefined) state.addDropPct = dropPct === '1' ? 1 : Number(dropPct);
        if (risePct !== undefined) state.addRisePct = risePct === '1' ? 1 : Number(risePct);
        if (stockEvent) {
          const set = new Set(state.addStockEvents);
          if (set.has(stockEvent)) set.delete(stockEvent); else set.add(stockEvent);
        ...
        if (parent) {
          for (const c of parent.querySelectorAll('button')) {
            const active =
              c.dataset.addPreset === state.addPreset ||
    ...
    if (action === 'paste-clipboard-add') {
      try {
        ...
        if (nodes.asinInput) nodes.asinInput.value = text;
        await submitAddProduct();
      } catch {
        if (nodes.addResultBox) nodes.addResultBox.textContent = 'Brak dostępu do schowka.';
      }
      return;
    }
    if (action === 'set-add-mode') {
      const mode = String(actionBtn.dataset.addMode || 'quick') as AddMode;
  ...
  document.body.addEventListener('click', (event) => {
    const target = event.target as HTMLElement;
    const chip = target.closest<HTMLButtonElement>('button.deal-chip[data-action]');
    if (!chip) return;
    if (!chip.closest('#dt-settings')) return;
    ...
    if (action === 'set-detail-profile') {
      ...
      const profiles: Record<string, { priceTypes: string[]; trackDrop: boolean; trackRise: boolean; dropPct: number; risePct: number; stockOut: boolean; stockBack: boolean }> = {
      ...
      settings.querySelectorAll<HTMLButtonElement>('button.deal-chip[data-action="set-detail-profile"]').forEach((c) => {
        c.classList.toggle('on', c.dataset.profile === profile);
      });

      // Apply price types
      settings.querySelectorAll<HTMLButtonElement>('button.deal-chip[data-action="set-detail-price-type"]').forEach((c) => {
        c.classList.toggle('on', cfg.priceTypes.includes(c.dataset.pt || ''));
      });

      // Apply track modes
      settings.querySelectorAll<HTMLButtonElement>('button.deal-chip[data-action="set-detail-track-mode"]').forEach((c) => {
        const mode = c.dataset.mode;
        c.classList.toggle('on', (mode === 'drop' && cfg.trackDrop) || (mode === 'rise' && cfg.trackRise));
      });

      // Apply drop preset
      settings.querySelectorAll<HTMLButtonElement>('button.deal-chip[data-action="set-detail-drop-preset"]').forEach((c) => {
        c.classList.toggle('on', Number(c.dataset.preset) === cfg.dropPct);
      });

      // Apply rise preset
      settings.querySelectorAll<HTMLButtonElement>('button.deal-chip[data-action="set-detail-rise-preset"]').forEach((c) => {
        c.classList.toggle('on', Number(c.dataset.preset) === cfg.risePct);
      });

      // Apply stock events
      settings.querySelectorAll<HTMLButtonElement>('button.deal-chip[data-action="set-detail-stock-event"]').forEach((c) => {
    ...
    if (action === 'set-detail-price-type') {
      ...
      const selected = settings.querySelectorAll<HTMLButtonElement>('button.deal-chip[data-action="set-detail-price-type"].on');
      if (selected.length === 0) chip.classList.add('on');
      return;
    }
    ...
    if (action === 'set-detail-drop-preset') {
      ...
      const dropInput = document.querySelector<HTMLInputElement>('#d-drop-pct');
    ...
    if (action === 'set-detail-rise-preset') {
      ...
      const preset = chip.dataset.preset;
      const riseInput = document.querySelector<HTMLInputElement>('#d-rise-pct');
  ...
  document.querySelector('.notif-tabs')?.addEventListener('click', (event) => {
    const target = event.target as HTMLElement;
    const tab = target.closest<HTMLDivElement>('.ntab[data-notif-tab]');
    if (!tab) return;
    const key = String(tab.dataset.notifTab || '') as NotifTab;
    state.notifTab = key;
  ...
  function updateSettingsSummaries() {
    ...
    if (appearanceSummary) {
      const theme = (document.querySelector('[data-change-action="set-theme"]') as HTMLSelectElement)?.value || 'AMOLED';
  ...
  document.querySelector('#v-settings')?.addEventListener('click', async (event) => {
    const target = event.target as HTMLElement;
    const accordionHead = target.closest<HTMLElement>('[data-action="toggle-settings-accordion"]');
    if (accordionHead) {
    ...
    if (action === 'run-scan-now') {
      try { await client.runScanNow(); } catch { /* noop */ }
      await refreshAllSettingsData();
      return;
    }
    ...
    if (action === 'install-pwa') {
      if (state.pwaInstallEvent) {
        (state.pwaInstallEvent as any).prompt();
      }
      return;
    }
    ...
    if (action === 'toggle-notif' || action === 'toggle-pref-saveall') {
      const toggle = target.closest<HTMLElement>('.toggle-switch') || actionBtn.querySelector('.toggle-switch') || actionBtn.closest<HTMLElement>('.settings-item')?.querySelector('.toggle-switch');
      if (toggle) {
        toggle.classList.toggle('on');
      }
      const prefs: Record<string, boolean> = {};
      document.querySelectorAll<HTMLElement>('#v-settings .toggle-switch[data-notif-key], #v-settings .toggle-switch[id^="tog-"]').forEach((el) => {
        const key = el.getAttribute('data-notif-key') || el.id.replace('tog-', '');
        if (key) prefs[key] = el.classList.contains('on');
      });
      if (state.chatId) {
    ...
    if (action === 'toggle-card-size') {
      ...
      if (label) label.textContent = document.body.classList.contains('cards-normal') ? 'Duży' : 'Normalny';
      updateSettingsSummaries();
      return;
    }
    if (action === 'apply-font-preset') {
      ...
      if (preset === 'compact') document.body.classList.add('font-compact');
      if (preset === 'readable') document.body.classList.add('font-readable');
      if (preset === 'standard') {
        document.querySelectorAll<HTMLElement>('.view[id^="v-"]').forEach((el) => {
          el.style.transform = '';
          el.style.width = '';
        });
        document.querySelectorAll<HTMLSelectElement>('.tab-font-scale-select').forEach((el) => {
          el.value = '1.0';
        });
      }
      updateSettingsSummaries();
    ...
    if (action === 'reset-tab-font-scale') {
      ...
      document.querySelectorAll(`[data-font-tab="${tab}"] .tab-font-scale-select`).forEach((el) => {
        (el as HTMLSelectElement).value = '1.0';
      });
      const tabEl = document.querySelector(`#v-${tab}`) as HTMLElement | null;
      if (tabEl) {
  ...
  document.querySelector('#v-settings')?.addEventListener('change', async (event) => {
    const target = event.target as HTMLElement;
    const changeEl = target.closest<HTMLElement>('[data-change-action]');
    if (!changeEl) return;
    const changeAction = changeEl.dataset.changeAction;
    if (changeAction === 'set-theme') {
      const theme = (changeEl as HTMLSelectElement).value;
    ...
    if (changeAction === 'set-tab-font-scale') {
      ...
      const value = (changeEl as HTMLSelectElement).value;
      const el = document.querySelector(`#v-${tab}`) as HTMLElement | null;
      if (el) {
...
async function bootstrap() {
  ...
  if (tg) {
    ...
    if (tg.initDataUnsafe?.user?.id) {
      ...
      if (tgId && tgId !== state.chatId) {
        ...
        if (nodes.chatIdInput) nodes.chatIdInput.value = tgId;
        await loadTrackings();
      }
    }
  ...
  setInterval(() => {
    ...
    const targetContainers = previews.filter((p) =>
      p.textContent?.includes('Cena docelowa')
    );
    ...
    const alertEditBtn = settings.querySelector('.detail-alert-status-btn') as HTMLElement | null;
    if (alertEditBtn) {
