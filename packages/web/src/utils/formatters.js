...
export function marketFlag(domainRaw) {
...
export function currencyForDomain(domainRaw) {
    return String(domainRaw || '').toLowerCase() === 'uk' ? '£' : '€';
}
export function formatPrice(value, domainRaw) {
...
export function parseAsinFromInput(raw) {
...
export function sortedMarkets(rows = []) {
...
export function bestNewPrice(item) {
...
export function getBestDomain(item) {
    ...
    if (rows.length)
        return String(rows[0].market || 'de').toLowerCase();
    return 'de';
}
export function avgNewPrice(item) {
