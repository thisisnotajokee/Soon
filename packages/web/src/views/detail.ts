import type { TrackingItem, ProductDetail, DetailChartRow, DetailTab } from '../state/types.js';
...
import { cardHistorySvg, formatChartLabelDate, formatChartAxisPrice, chartToken } from './shared.js';

declare const Chart: any;
let detailChartInstance: any = null;

export function buildDetailChartRows(item: TrackingItem, detail: ProductDetail | null) {
...
export function applyDetailChartFilters(rows: DetailChartRow[], selectedMarkets: string[], rangeDays: number) {
...
export function buildDetailChartDatasets(rows: DetailChartRow[]) {
  const datasets: any[] = [];
...
export function computeDetailChartStats(rows: DetailChartRow[]) {
  const points = rows
    .map((row) => {
...
export function renderDetailChartCanvas(filteredRows: DetailChartRow[]) {
...
export function normalizeDetailHistorySeries(item: TrackingItem, detail: ProductDetail | null) {
...
export function getDetailChartMarkets(asinRaw: string, defaultMarkets: string[], bestDomainRaw: string) {
  ...
  if (savedUnique.length) return savedUnique;
...
export function detailChartSvg(
...
export function renderPriceRows(rows: Array<[string, number]>, type: 'new' | 'used', asin: string) {
...
export function renderMarketCompare(rowsSource: Array<[string, number]>, asin: string, mode: 'new' | 'used' = 'new') {
...
export async function hydrateDetail(asinRaw: string) {
...
export function normalizeThresholdPercent(value: string | number | null | undefined) {
  const raw = toNullableNumber(value);
  if (raw === null) return null;
  if (Math.abs(raw) > 100) return Number((raw / 100).toFixed(2));
  return raw;
}

export function normalizeThresholdTargetPrice(value: string | number | null | undefined) {
  const raw = toNullableNumber(value);
  if (raw === null) return null;
  if (Math.abs(raw) >= 100000) return Number((raw / 10000).toFixed(2));
  return raw;
}
...
export function renderDetail(item: TrackingItem) {
  ...
  const defaultChartMarkets = DETAIL_MARKETS.filter((market) =>
    marketRows.some((row) => {
      const domain = String(row.market || '').toLowerCase();
      if (domain !== market) return false;
  ...
  const hasNumericThreshold =
    (Number.isFinite(Number(thresholdDropPct)) && Number(thresholdDropPct) > 0) ||
  ...
  if (nodes.detailTopbarTitle) nodes.detailTopbarTitle.textContent = String(item.title || asin || copy.details);
}

export async function openDetail(asinRaw: string) {
  const asin = String(asinRaw || '').trim().toUpperCase();
...
export async function refreshSelectedTracking(asinRaw: string) {
  const asin = String(asinRaw || '').trim().toUpperCase();
  if (!asin) return;
  try {
...
export async function snoozeSelectedTracking(asinRaw: string) {
  const asin = String(asinRaw || '').trim().toUpperCase();
  if (!asin) return;
...
export async function unsnoozeSelectedTracking(asinRaw: string) {
  const asin = String(asinRaw || '').trim().toUpperCase();
  if (!asin) return;
