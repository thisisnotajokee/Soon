...
export async function refreshAllSettingsData() {
    ...
    try {
        ...
        if (nodes.setLastscan && health?.lastScan)
            nodes.setLastscan.textContent = formatDateTime(health.lastScan);
        if (nodes.setNextscan && health?.nextScan)
            nodes.setNextscan.textContent = formatDateTime(health.nextScan);
        if (nodes.setLastscanDesc)
            nodes.setLastscanDesc.textContent = 'Ostatni skan cen';
        if (nodes.setNextscanDesc)
            nodes.setNextscanDesc.textContent = 'Następny zaplanowany';
        if (nodes.setKeepa)
            nodes.setKeepa.textContent = health?.keepa === false ? 'OFF' : 'Aktywne';
        if (nodes.setHunter)
            nodes.setHunter.textContent = health?.hunter === false ? 'OFF' : 'Aktywny';
    }
    catch {
        // noop
    }
