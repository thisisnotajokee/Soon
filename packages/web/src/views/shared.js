...
import { escapeHtml, currencyForDomain } from '../utils/formatters.js';
import { renderDetail } from './detail.js';
export function chartToken(points, prefix) {
...
export function sparklineSvg(points, stroke = '#ff7a00', domainRaw = 'de') {
    if (!points.length) {
...
export function formatChartLabelDate(value) {
...
export function formatChartAxisPrice(value, domainRaw) {
...
export function cardHistorySvg(points, domainRaw) {
    if (!points.length) {
...
export function openAmazon(asinRaw, marketRaw) {
...
export function applyCopy() {
    ...
    if (nodes.searchInput)
        nodes.searchInput.placeholder = copy.searchPlaceholder;
    if (nodes.navLabels.tracking)
        nodes.navLabels.tracking.textContent = copy.nav.tracking;
    if (nodes.navLabels.deals)
        nodes.navLabels.deals.textContent = copy.nav.deals;
    if (nodes.navLabels.add)
        nodes.navLabels.add.textContent = copy.nav.add;
    if (nodes.navLabels.alerts)
        nodes.navLabels.alerts.textContent = copy.nav.alerts;
    if (nodes.navLabels.settings)
        nodes.navLabels.settings.textContent = copy.nav.settings;
    ...
    if (dealsTitle)
        dealsTitle.textContent = copy.dealsTitle;
    if (dealsDesc)
        dealsDesc.textContent = copy.dealsDesc;
    if (addTitle)
        addTitle.textContent = copy.addTitle;
    if (addDesc)
        addDesc.textContent = copy.addDesc;
    if (alertsTitle)
        alertsTitle.textContent = copy.alertsTitle;
    if (alertsDesc)
        alertsDesc.textContent = copy.alertsDesc;
    if (chatIdSave)
        chatIdSave.textContent = copy.setChatId;
