import type { TrackingItem, TrackingStatusFilter } from '../state/types.js';
...
import {
  escapeHtml, marketFlag, formatPrice, formatDateTime,
  bestNewPrice, bestUsedPrice, getBestDomain, isSnoozed, getSparkline,
  normalizedRows,
...
export function visibleItems() {
  ...
  const list = state.trackings.filter((item) => {
    ...
    if (sort === 'stock_used') {
      if (bestUsedPrice(item) === null) return false;
    }
    if (sort === 'with_alerts') {
      if (!item.cardPreview?.outOfStock && !item.cardPreview?.popularity) return false;
    }
...
export function recommendationBadge(item: TrackingItem) {
  const copy = t();
  const delta = Number(item.cardPreview?.deltaPctVsAvg);
...
export function renderTrackingCard(item: TrackingItem) {
  const copy = t();
